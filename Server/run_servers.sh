...
MACHINES=(tr-open-02 tr-open-03 tr-open-04 tr-open-05)
...
tmux new-session \; \
	...
	select-pane -t 0 \; \
	send-keys "ssh -t ${MACHINES[3]} \"cd $(pwd) > /dev/null; echo -n 'Connected to '; hostname; sleep .5s; cd ../MiddleWare ; ./run_middle.sh ${MACHINES[0]} ${MACHINES[1]} ${MACHINES[2]}\"" C-m \;
